...
Date.parseStandard = function(input) {
  var error, hours, minutes, ref, seconds;
  try {
    ref = input.split('-'), hours = ref[0], minutes = ref[1], seconds = ref[2];
    return new Date(parseInt(hours, 10), parseInt(minutes, 10) - 1, parseInt(seconds, 10));
  } catch (error) {
    return new Date(-1);
  }
};
